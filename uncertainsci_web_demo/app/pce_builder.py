...
from UncertainSCI.distributions import (
    BetaDistribution,
    UniformDistribution,
...
def initialize(server):
    ...
    def build_model():
        ...
        param_dict = {
            ...
            "domain": [
                [
                    state.dist_domain_0[0],
                    state.dist_domain_1[0],
                    state.dist_domain_2[0],
                ],
                [
                    state.dist_domain_0[1],
                    state.dist_domain_1[1],
                    state.dist_domain_2[1],
                ],
            ],
        }
        ...
        # evaluate switcher
        x, model_output = make_model(
            state.model_name, state.Nparams, state.N, pce.samples
        )
...
def make_NormalDistribution(param_dict):
    return NormalDistribution(
        mean=np.array(param_dict["mean"]), cov=np.array(param_dict["cov"])
    )
...
def make_ExponentialDistribution(param_dict):
    ...
    return ExponentialDistribution(
        lbd=np.array(param_dict["lbd"]), loc=np.array(param_dict["loc"])
    )
...
MODELS = {"1D Lapace Ode": make_lapace_ode_1d, "Sine Modulation": make_sine_modulation}
